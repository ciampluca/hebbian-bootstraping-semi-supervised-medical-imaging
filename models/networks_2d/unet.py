...
class UNet_Transposed(nn.Module):
    def __init__(self, in_chns, class_num):
        super(UNet_Transposed, self).__init__()

        self.Maxpool = nn.MaxPool2d(kernel_size=2, stride=2)

        self.Conv1 = ConvBlock(ch_in=in_chns, ch_out=64)
        self.Conv2 = ConvBlock(ch_in=64, ch_out=128)
        self.Conv3 = ConvBlock(ch_in=128, ch_out=256)
        self.Conv4 = ConvBlock(ch_in=256, ch_out=512)
        self.Conv5 = ConvBlock(ch_in=512, ch_out=1024)
...
class UNet_Transposed_Leaky(nn.Module):
    ...
    def forward(self, x):
        ...
        main_seg = self.out_conv(main_seg)
...
def unet_transp(in_channels, num_classes):
    model = UNet_Transposed(in_channels, num_classes)
