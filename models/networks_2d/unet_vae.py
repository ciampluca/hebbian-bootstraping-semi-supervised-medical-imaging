...
class UNet_Transposed_Leaky(nn.Module):
    ...
    def __init__(self, in_chns, class_num, test_unsup_low_layer=False, linear_probe=True, multiple_layers=True):
        ...
        self.var = nn.Conv2d(256, 256, kernel_size=1)
        ...
        self.reconstr = nn.Conv2d(params['feature_chns'][0], in_chns, kernel_size=1)

    def forward(self, x):
        feature = self.encoder(x)
        
        # Sampling
        mu = self.mu(feature[-1])
        log_var = self.var(feature[-1])
        std = torch.exp(0.5 * log_var)
        eps = torch.randn_like(std)
        latent =  eps * std + mu        
        
        dec_out = self.main_decoder(feature, latent)
        main_seg = self.out_conv(dec_out)
        reconstr = self.reconstr(dec_out)
        ...
        output = {'output': main_seg, 'mu': mu, 'log_var': log_var, 'reconstr': reconstr}
...
def unet_vae(in_channels, num_classes, initialization_weights='kaiming'):
