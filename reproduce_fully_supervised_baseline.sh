...
for DATASET in ${DATASETS_3D[@]}; do
    for REP in $(seq $(( $START_REP )) $(( $REPS - 1 ))); do
        ...
        #python test_3d.py --dataset_name $DATASET --network unet --batch_size $EVAL_BATCH_SIZE --path_dataset $DATA_ROOT/$DATASET --best JI --path_exp $EXP_ROOT/$DATASET/fully_sup/unet/inv_temp-1/regime-100/run-$REP
    done
done