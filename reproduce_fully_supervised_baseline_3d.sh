        ...
        for REP in $(seq $(( $START_REP )) $(( $REPS - 1 ))); do
            case $DATASET in
                Atrial)
                    ...
                    ;;
                LiTS)
                    python train_sup_3d.py --dataset_name $DATASET --network $NETWORK --path_dataset $DATA_ROOT/$DATASET --path_root_exp $EXP_ROOT --regime 100 --batch_size $BATCH_SIZE --optimizer $OPTIMIZER --seed $REP --validate_iter 2 --device $GPU --lr $LR --loss dice --patch_size "(112, 112, 32)"
                    python test_3d.py --dataset_name $DATASET --network $NETWORK --batch_size $EVAL_BATCH_SIZE --path_dataset $DATA_ROOT/$DATASET --best JI --path_exp $EXP_ROOT/$DATASET/fully_sup/$NETWORK/inv_temp-1/regime-100/run-$REP --patch_size "(112, 112, 32)" --patch_overlap "(56, 56, 16)" --device $GPU
                    ;;            
            esac
        done
    done
