...
for DATASET in ${DATASETS[@]}; do
    for NETWORK in ${NETWORKS[@]}; do
        for REP in $(seq $(( $START_REP )) $(( $REPS - 1 ))); do
            ...
            python test_3d.py --dataset_name $DATASET --network $NETWORK --batch_size $EVAL_BATCH_SIZE --path_dataset $DATA_ROOT/$DATASET --best JI --path_exp $EXP_ROOT/$DATASET/fully_sup/kaiming_$NETWORK/inv_temp-1/regime-100/run-$REP --patch_size "(96, 96, 80)" --patch_overlap "(48, 48, 20)" --device $GPU
        done
    done
done