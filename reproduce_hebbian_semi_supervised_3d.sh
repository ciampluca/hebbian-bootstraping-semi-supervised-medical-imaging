            ...
            for NETWORK in ${NETWORKS[@]}; do
                for HEBB_MODE in ${HEBB_MODES[@]}; do
                    for REP in $(seq $(( $START_REP )) $(( $REPS - 1 ))); do
                        ...
                        python test_3d.py --dataset_name $DATASET --network $NETWORK"_dtc" --batch_size $EVAL_BATCH_SIZE --path_dataset $DATA_ROOT/$DATASET --best JI --path_exp $EXP_ROOT/$DATASET/semi_sup/h_dtc_"$NETWORK"_"$HEBB_MODE"/inv_temp-$K/regime-$REGIME/run-$REP --patch_size "(96, 96, 80)" --patch_overlap "(48, 48, 20)" --hebbian_pretrain True --device $GPU                     
                    done
                done
            done
        done
