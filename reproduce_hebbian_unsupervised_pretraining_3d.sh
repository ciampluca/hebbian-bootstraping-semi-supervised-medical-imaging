...
EXCLUDE_LAYER="conv dsv1 dsv2 dsv3 dsv4 out_conv out_sdf out_seg"
            ...
            for HEBB_MODE in ${HEBB_MODES[@]}; do
                case $DATASET in
                    Atrial)
                        ...
                        python pretrain_hebbian_unsup_3d.py --dataset_name $DATASET --network $NETWORK"_"dtc --path_dataset $DATA_ROOT/$DATASET --path_root_exp $EXP_ROOT --batch_size $BATCH_SIZE --optimizer $OPTIMIZER --seed 0 --validate_iter $VALIDATE_ITER --device $GPU --lr $LR --loss dice --patch_size "(96, 96, 80)" --hebb_mode $HEBB_MODE --hebb_inv_temp $K --exclude $EXCLUDE_LAYER #--threshold $THRESHOLD
                        python test_3d.py --dataset_name $DATASET --network $NETWORK"_"dtc --batch_size $EVAL_BATCH_SIZE --path_dataset $DATA_ROOT/$DATASET --best last --path_exp $EXP_ROOT/$DATASET/hebbian_unsup/$NETWORK"_dtc_"$HEBB_MODE/inv_temp-$K/regime-100/run-0 --patch_size "(96, 96, 80)" --patch_overlap "(48, 48, 40)" --hebbian_pretrain True --device $GPU #--threshold $THRESHOLD                         
                        ;;  
                    LiTS)
                        ...
                        python pretrain_hebbian_unsup_3d.py --dataset_name $DATASET --network $NETWORK"_"dtc --path_dataset $DATA_ROOT/$DATASET --path_root_exp $EXP_ROOT --batch_size $BATCH_SIZE --optimizer $OPTIMIZER --seed 0 --validate_iter $VALIDATE_ITER --device $GPU --lr $LR --loss dice --patch_size "(112, 112, 32)" --hebb_mode $HEBB_MODE --hebb_inv_temp $K --exclude $EXCLUDE_LAYER --samples_per_volume_train 8 --samples_per_volume_val 12 #--threshold $THRESHOLD
                        python test_3d.py --dataset_name $DATASET --network $NETWORK"_"dtc --batch_size $EVAL_BATCH_SIZE --path_dataset $DATA_ROOT/$DATASET --best last --path_exp $EXP_ROOT/$DATASET/hebbian_unsup/$NETWORK"_dtc_"$HEBB_MODE/inv_temp-$K/regime-100/run-0 --patch_size "(112, 112, 32)" --patch_overlap "(56, 56, 16)" --hebbian_pretrain True --device $GPU --postprocessing True --fill_hole_thr 100 #--threshold $THRESHOLD                          
                        ;;
                esac
            done
        done
