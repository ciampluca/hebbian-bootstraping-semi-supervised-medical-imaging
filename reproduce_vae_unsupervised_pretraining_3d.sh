    ...
    for NETWORK in ${NETWORKS[@]}; do
            case $DATASET in
                Atrial)
                    ...
                    ;;  
                LiTS)
                    python pretrain_vae_unsup_3d.py --dataset_name $DATASET --network $NETWORK --path_dataset $DATA_ROOT/$DATASET --path_root_exp $EXP_ROOT --batch_size $BATCH_SIZE --optimizer $OPTIMIZER --seed 0 --validate_iter $VALIDATE_ITER --device $GPU --lr $LR --loss dice --patch_size "(112, 112, 32)" --samples_per_volume_train 8 --samples_per_volume_val 12 #--threshold $THRESHOLD
                    python test_3d.py --dataset_name $DATASET --network $NETWORK --batch_size $EVAL_BATCH_SIZE --path_dataset $DATA_ROOT/$DATASET --best last --path_exp $EXP_ROOT/$DATASET/vae_unsup/$NETWORK/inv_temp-1/regime-100/run-0 --patch_size "(112, 112, 32)" --patch_overlap "(56, 56, 16)" --device $GPU --postprocessing True --fill_hole_thr 100 #--threshold $THRESHOLD
                    ;;
            esac
        done
done
