    ...
    if args.regime < 100:
        if args.load_hebbian_weights:
            ...
            if args.init_weights != "kaiming":
                path_run = os.path.join(args.path_root_exp, os.path.split(args.path_dataset)[1], "semi_sup", "{}_dtc_{}".format(args.init_weights, net_name), "inv_temp-1", "regime-{}".format(args.regime), "run-{}".format(args.seed))
            else:
