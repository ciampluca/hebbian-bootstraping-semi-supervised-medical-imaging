...
if __name__ == '__main__':
    ...
    if args.regime < 100:
        if args.load_hebbian_weights:
            path_run = os.path.join(args.path_root_exp, os.path.split(args.path_dataset)[1], "semi_sup", "h_raddino_decoder_{}_{}".format(args.network, args.hebbian_rule), "inv_temp-{}".format(args.hebb_inv_temp), "regime-{}".format(args.regime), "run-{}".format(args.seed))
        else:
            if args.init_weights != "kaiming":         
                path_run = os.path.join(args.path_root_exp, os.path.split(args.path_dataset)[1], "semi_sup", "{}_raddino_decoder_{}".format(args.init_weights, args.network), "inv_temp-1", "regime-{}".format(args.regime), "run-{}".format(args.seed))
            else:
                path_run = os.path.join(args.path_root_exp, os.path.split(args.path_dataset)[1], "semi_sup", "raddino_decoder_{}".format(args.network), "inv_temp-1", "regime-{}".format(args.regime), "run-{}".format(args.seed))
    else:
        path_run = os.path.join(args.path_root_exp, os.path.split(args.path_dataset)[1], "fully_sup", "raddino_decoder_{}".format(args.network), "inv_temp-1", "regime-{}".format(args.regime), "run-{}".format(args.seed))
    ...
    for epoch in range(args.num_epochs):
        ...
        for i in range(num_batches['train_sup']):
            ...
            with torch.inference_mode():
                img_train_unsup = reshape_patch_embeddings(encoder(img_train_unsup).last_hidden_state)
            pred_train_unsup = model(img_train_unsup.clone())
            ...
            with torch.inference_mode():
                img_train_sup = reshape_patch_embeddings(encoder(img_train_sup).last_hidden_state)
            pred_train_sup = model(img_train_sup.clone())
            ...
            print("***************** " + str(pred_train_sup.shape))
        ...
        if count_iter % args.validate_iter == 0:
            ...
            with torch.no_grad():
                ...
                for i, data in enumerate(dataloaders['val']):
                    ...
                    with torch.inference_mode():
                        inputs_val = reshape_patch_embeddings(encoder(inputs_val).last_hidden_state)

                    outputs_val = model(inputs_val.clone())
