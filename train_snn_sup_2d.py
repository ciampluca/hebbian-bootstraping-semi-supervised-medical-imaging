...
if __name__ == '__main__':
    ...
    parser.add_argument('--hebb_inv_temp', default=1, type=float, help='hebbian temp')
    ...
    if args.regime < 100:
        if args.load_hebbian_weights:
            ...
            path_run = os.path.join(args.path_root_exp, os.path.split(args.path_dataset)[1], "semi_sup", "snn", "inv_temp-1", "regime-{}".format(args.regime), "run-{}".format(args.seed))
        else:
            path_run = os.path.join(args.path_root_exp, os.path.split(args.path_dataset)[1], "semi_sup", "snn", "inv_temp-1", "regime-{}".format(args.regime), "run-{}".format(args.seed))
    else:
        path_run = os.path.join(args.path_root_exp, os.path.split(args.path_dataset)[1], "fully_sup", "snn", "inv_temp-1", "regime-{}".format(args.regime), "run-{}".format(args.seed))
    ...
    # create model
    from snn_models import snn_vgg
    model = snn_vgg.SNN_VGG(img_size=128, input_dim=cfg['IN_CHANNELS'], num_cls=cfg['NUM_CLASSES']) #get_network(args.network, cfg['IN_CHANNELS'], cfg['NUM_CLASSES'], args.init_weights)
